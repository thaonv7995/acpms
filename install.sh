...
print_success_report() {
  ...
  printf "${C_GREEN}║${C_RESET}  ${C_CYAN}%-15s${C_RESET}  ${C_BOLD}%s${C_RESET}\n" "Access URL" "$url"
  printf "${C_GREEN}║${C_RESET}  ${C_CYAN}%-15s${C_RESET}  %s\n" "Admin email" "$email"
  [ -n "$has_pass" ] && printf "${C_GREEN}║${C_RESET}  ${C_CYAN}%-15s${C_RESET}  ${C_YELLOW}%s${C_RESET}  ${C_DIM}(save it; not shown again)${C_RESET}\n" "Admin password" "${ACPMS_ADMIN_PASSWORD}"
  printf "${C_GREEN}║${C_RESET}  ${C_CYAN}%-15s${C_RESET}  %s\n" "Config" "$ENV_FILE"
