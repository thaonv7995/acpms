...
function getStatusIcon(status?: string) {
  ...
  if (s === 'completed')
    return <Check aria-hidden className="h-4 w-4 text-green-600 dark:text-green-400" />;
  if (s === 'in_progress' || s === 'in-progress')
    return <CircleDot aria-hidden className="h-4 w-4 text-blue-500 dark:text-blue-400" />;
  if (s === 'cancelled')
    return <Circle aria-hidden className="h-4 w-4 text-gray-400 dark:text-gray-600" />;
  return <Circle aria-hidden className="h-4 w-4 text-gray-400 dark:text-gray-600" />;
}
  ...
  return (
    ...
    >
      <summary className="list-none cursor-pointer">
        <Card className="bg-gray-50 dark:bg-gray-900 p-3 text-sm flex items-center justify-between">
          <span className="font-medium">Todos ({todos.length})</span>
          <ChevronUp
            aria-hidden
            className="h-4 w-4 text-gray-500 dark:text-gray-400 transition-transform group-open:rotate-180"
          />
        </Card>
      </summary>
